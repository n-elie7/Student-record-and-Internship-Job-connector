...
class RealtimeListener:
    """Handles real-time database change notifications"""
    def __init__(self):
        self.realtime_flags = {"students": False, "internships": False, "applications": False}
        self.change_detected = False

    def _payload_printer(self, payload):
        try:
            ...
            if record:
                print(f"   → {record}")
            if table in self.realtime_flags:
                self.realtime_flags[table] = True
            self.change_detected = True
        except Exception as e:
            print(f"[Realtime] Error: {e}")


    async def async_realtime_listener(self):
        ...
        for tbl in ("students", "internships", "applications"):
            for ev in ("INSERT", "UPDATE", "DELETE"):
                channel.on_postgres_changes(
                    ...
                    schema="public",
                    table=tbl,
                    callback=lambda payload, t=tbl, e=ev: self._payload_printer(
                        {
                            "table": t,
                            "event": e,
    ...
    def start_realtime_listener(self):
        ...
        def run_async():
            ...
            try:
                loop.run_until_complete(self.async_realtime_listener())
            except Exception as e:
                print(f"[Realtime] Error: {e}")
